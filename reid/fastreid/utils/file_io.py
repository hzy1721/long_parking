...
class PathManager:
    ...
    def open(
        ...
        """
        Open a stream to a URI, similar to the built-in `open`. 打开文件。
        Args:
            path (str): A URI supported by this PathHandler
            mode (str): Specifies the mode in which the file is opened. It defaults
                to 'r'.
    ...
    def copy(
        ...
        """
        Copies a source path to a destination path. 复制文件。
        Args:
            src_path (str): A URI supported by this PathHandler
            dst_path (str): A URI supported by this PathHandler
            overwrite (bool): Bool flag for forcing overwrite of existing file
    ...
    def exists(path: str, **kwargs: Any) -> bool:
        """
        Checks if there is a resource at the given URI. 检查URI指向的文件是否存在。
        Args:
            path (str): A URI supported by this PathHandler
        Returns:
            bool: true if the path exists
    ...
    def isdir(path: str, **kwargs: Any) -> bool:
        """
        Checks if the resource at the given URI is a directory. 是否为目录。
        Args:
            path (str): A URI supported by this PathHandler
        Returns:
            bool: true if the path is a directory
    ...
    def mkdirs(path: str, **kwargs: Any) -> None:
        ...
        Recursive directory creation function. Like mkdir(), but makes all
        intermediate-level directories needed to contain the leaf directory.
        Similar to the native `os.makedirs`. 创建目录。
        Args:
            path (str): A URI supported by this PathHandler
        """
