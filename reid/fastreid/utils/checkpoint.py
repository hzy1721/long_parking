...
class Checkpointer(object):
    ...
    def __init__(
        ...
        """
        Args:
            model (nn.Module): model. 模型对象。
            save_dir (str): a directory to save and find checkpoints. 模型文件所在的目录。
            save_to_disk (bool): if True, save checkpoint to disk, otherwise
                disable saving for this checkpointer. 是否把模型保存到磁盘。
            checkpointables (object): any checkpointable objects, i.e., objects
                that have the `state_dict()` and `load_state_dict()` method. For
                example, it can be used like
                `Checkpointer(model, "dir", optimizer=optimizer)`.
                若干个可保存的对象，必须有state_dict()和load_state_dict()方法，以关键字参数的形式传入。
        """
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> object:
        ...
        function has to be called on all ranks. 从模型文件加载权重。

        Args:
            path (str): path or url to the checkpoint. If empty, will not load
                anything. 权重文件的路径或URL。
    ...
    def _load_file(self, f: str):
        ...
        different formats. 用torch.load加载本地文件。
